...
Most of the data variables are binary, such as whether the subject was black or not. Even when the variables are not binary, the pdf does not exhibit the shape of a normal distribution.

(b) Estimate your full regression model and show your results (e.g., the output from the `summary' command in R). Discuss these results. 
```{r}
y = lm(re78~trt+age+educ+black)
summary(y)
```

I did not use the all the predictors/variables since I thought that some of them are statistically insignificant. Since the p-values are so small, the estimated coefficients that we obtained by running the regression is statistically significant. However since the R^2 is so low, the model does not fit the data.
...
From the Mallows Cp, we can see that the choosing all 4 variables would give the lowest Cp. However, the Cp is sufficiently low enough when we just take trt, age and educ variables. So I'll use these variables.
...
```{r}
plot(vif(y1),col="skyblue3",xlab="Variables",ylab="Residuals",main="Variance Inflation Factor",cex.axis=0.8,cex.main=0.9)
```

The VIF for variables trt, age and educ are 1.092, 1.108 and 1.078, which are very small. Therefore, they are not collinear.
...
The plot shows that there is very little correlation between any 2 variables from the data, except that between years of education and being a high school dropout, and between the different years of the earning. This might imply that the other variables such as age, years of education, etc does not affect earnings in the population.
...
From the plot, we could see that there are outliers. One way to deal with it is to consider modifying the model, such as to a log-linear one. Since we could see that there is only one outlier that stands out too much, another way is to just remove that point.
...
Since pdf of residuals closely resembles that of a normal distribution, we know that the OLS is unbiased.This means that we can estimate the coefficients of the variables without bias.
...
Since the shape of the line is slope slightly upwards, maybe the linear model does not fit the data well. However we could that there are some points that jump at the right tail, which implies that we have outliers.
...
The way the plots scatter all over indicates that there is very little correspondence between the observed values and the predicted values. This means that the model for prediction is poor.

##Problem 2
clear all variables and prior sessions and get the data and attach it
```{r}
rm(list=ls(all=TRUE))
...
Obtain the descriptive statistics: mean, median, variance, standard deviation, skewness, and kurtosis.
```{r}
...
This statistics shows that the median is higher slightly compared to the mean, the data is skewed slightly to the right, which can be verified by looking at the negative skewness and the shape of the histogram. The correlation between US GDP growth rate and SP500 return is also very low, which means a linear regression would not work for these 2 variables.
...
plot(rpce_pctchg, main="Scatterplot of real personal consumption expenditure growth rate", 
     ylab="consumption expenditure growth rate", pch=19)
plot(rdpi_pctchg, main="Scatterplot of real disposal personal income growth rate",  
     ylab="personal income growth rate", pch=19)
```

From the plots, we can see that growth rate of the real personal consumption expenditure growth rate is less volatile than that of real disposal personal income, which can be explained by the permanent income model. Permanent income model that the current expenditure is not determined just by their current income but also by their future expected income. As such, it would mean that even if a person's income varies, which some occasional outliers, his consumption might not vary since he could be saving the more for a rainy day.
...
both values for the intercept and the coefficient of income have very low p value, which means that they are statistically significant. The intercept is close to 0, which means that if the person's income is 0, he would not consume. The income coefficient is positive, which means that the higher the income, the more the person will consume

c)
```{r}
rdpi_pctchg_lag = c(NA,rdpi_pctchg[-length(rdpi_pctchg)])
y = lm(rpce_pctchg~rdpi_pctchg+rdpi_pctchg_lag)
summary(y)
```

There seems to be no adjustment lag in the real personal consumption expenditure since the regression shows the coefficients are still significant, which means that the the income at present and the lagged income is not collinear. This makes sense by the permanent income model, since even if you don't have any income for the next few days, you would still consume since you still have your savings from previously-earned income.

##Problem 4
clear all variables and prior sessions and obtain the data and create time series objects
```{r}
rm(list=ls(all=TRUE))
...
All 4 scatterplots of time series, i.e those of real GDP, Japan-US FOREX and Treasury Constant Maturity Rate, Civilian Unemployment Rate show no equal mean for its stochastic processes over time. This means that there these time series are neither first nor second order weakly stationary.
...
Obtain the house prices growth and the interest rate change
```{r}
hp_growth = diff(log(P))
ir_chg = diff(R..in...)
```

Obtain the ACF and PCF of house prices, interest rates, house prices growth and interest rate changes
```{r}
windows()
par(mfrow=c(2,2))
...
Although there is a very high ACF for the house prices and interest rates, the corresponding PACF is very low. But since the ACF slowly decreases as the number of lags increases, the ACF of the first lag propagates to those of all subsequent lags, which means that there is dependence when lag is 1 but not if 
for all other lags greater than 1 and thus there is strong dependence in the short term dynamics but not in the long term. This is further proved since the ACF and PACF of the house prices growth and interest rate changes show similar patterns to those of house prices and interest rates.
